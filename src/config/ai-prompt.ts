...
export const SCHEMA_DESCRIPTIONS = {
  ...
  tags: (allowedTags: string[]) =>
    "MUST BE AN ARRAY. Select up to 10 most relevant and specific tags from this list ONLY: " +
    allowedTags.join(", ") +
    ". Prioritize specificity and direct relevance to the model. Select tags from multiple categories when applicable: (1) Disease/condition (e.g., Malaria, Tuberculosis, Cancer). (2) Specific pathogen/organism (e.g., P.falciparum, E.coli, M.tuberculosis—select ONLY the exact organism(s) the model explicitly targets). (3) Biological target/protein (e.g., hERG, CYP450, BACE). (4) Molecular property/endpoint (e.g., IC50, Solubility, Toxicity, Permeability). (5) Data source (e.g., ChEMBL, TDCommons). (6) Computational method (e.g., Chemical graph model, Embedding, Fingerprint). Be specific: if a model is for E.coli, do NOT also tag other bacteria unless they are explicitly included. Choose the most specific tags that directly relate to what the model predicts or targets. Use EXACT matches only. Do not create new tags. If uncertain, use [].",

  publication_url:
    "MUST BE AN STRING.Link to the original publication. Prefer the journal page (DOI link) over PubMed, ResearchGate or other secondary sources. Format: full URL starting with https://. If not found in the PDF, leave empty.",

  source_url:
    "MUST BE AN STRING. Full URL to the model source. ACCEPTED FORMATS: (1) Code repository URL (GitHub, GitLab, Bitbucket, etc.) - must be complete repository URL (e.g., 'https://github.com/username/repo-name', NOT just 'https://github.com'). (2) Web application or web server hosting the model (e.g., 'https://example.com/model-tool'). (3) Third-party REST API endpoint documentation page. PRIORITY: Prefer code repositories over web apps when both are available. If the exact source URL is not found in the PDF, leave empty rather than providing an incomplete or generic URL (e.g., do not use just domain names without specific paths).",
